...
export function prometheusText() {
  const lines = [
    "# HELP hive_bridge_http_requests_total Mesh transport-bridge stub HTTP responses",
    "# TYPE hive_bridge_http_requests_total counter",
  ];
  for (const [k, n] of [...counts.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    lines.push(
      `hive_bridge_http_requests_total{method="${method}",path="${esc}",code="${code}"} ${n}`
    );
  }
  return `${lines.join("\n")}\n`;
}
