import { InMemoryAuditStore, HashChain } from './chunk-AXLQRD2C.js';
import { generateSigningKeyPair, generateNoiseKeyPair } from './chunk-TQHOONTS.js';
import { addHiveExtensions } from './chunk-6CT5KQ5Q.js';
import { compose, createServerContext } from './chunk-R5AGGCWU.js';
import { ServerCallContext } from './chunk-BFTDQG3V.js';
import { A2AError } from './chunk-TT7THEVJ.js';
import { v4 } from 'uuid';
import { z } from 'zod';
...
var DefaultRequestHandler = class {
  ...
  async _createRequestContext(incomingMessage, context) {
    ...
    const taskId = incomingMessage.taskId || v4();
    ...
    const contextId = incomingMessage.contextId || task?.contextId || v4();
  ...
  async *sendMessageStream(params, context) {
    ...
    this.agentExecutor.execute(requestContext, eventBus).catch((err) => {
      ...
      const errorTaskStatus = {
        kind: "status-update",
        taskId: requestContext.task?.id || v4(),
...
var PartSchema = z.discriminatedUnion("kind", [
...
var HiveA2AServer = class {
  ...
  constructor(config) {
    ...
    const signingKeyPair = config.signing !== false ? config.signing?.keyPair ?? generateSigningKeyPair() : void 0;
    const noiseKeyPair = config.noise !== false ? config.noise?.staticKeyPair ?? generateNoiseKeyPair() : void 0;
    enhancedCard = addHiveExtensions(
      enhancedCard,
...
export { DefaultExecutionEventBus, DefaultExecutionEventBusManager, DefaultPushNotificationSender, DefaultRequestHandler, ExecutionEventQueue, HiveA2AServer, HiveRequestHandler, InMemoryPushNotificationStore, InMemoryTaskStore, RequestContext, ResultManager };
