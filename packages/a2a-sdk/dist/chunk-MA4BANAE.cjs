...
function generateSigningKeyPair() {
  const secretKey = ed25519.ed25519.utils.randomPrivateKey();
  const publicKey = ed25519.ed25519.getPublicKey(secretKey);
...
function dh(secretKey, publicKey) {
  return ed25519.x25519.getSharedSecret(secretKey, publicKey);
}
...
function initiatorHandshake1(localStatic, remoteStaticPub) {
  ...
  const cipher1 = chacha.chacha20poly1305(ck1, ZERO_NONCE);
...
function responderHandshake(localStatic, msg1) {
  ...
  const decipher1 = chacha.chacha20poly1305(ck1, ZERO_NONCE);
...
exports.initiatorHandshake1 = initiatorHandshake1;
exports.initiatorHandshake2 = initiatorHandshake2;
exports.responderHandshake = responderHandshake;
exports.sign = sign;
