...
// src/core/server/transports/jsonrpc/jsonrpc_transport_handler.ts
  ...
  async handle(requestBody, context) {
    ...
    try {
      if (typeof requestBody === "string") {
        ...
        throw chunk6NYM5ZKZ_cjs.A2AError.parseError("Invalid request body type.");
      }
      if (!this.isRequestValid(rpcRequest)) {
        throw chunk6NYM5ZKZ_cjs.A2AError.invalidRequest("Invalid JSON-RPC Request.");
      }
    } catch (error) {
      const a2aError = error instanceof chunk6NYM5ZKZ_cjs.A2AError ? error : chunk6NYM5ZKZ_cjs.A2AError.parseError(
    ...
    try {
      if (method !== "agent/getAuthenticatedExtendedCard" && !this.paramsAreValid(rpcRequest.params)) {
        throw chunk6NYM5ZKZ_cjs.A2AError.invalidParams(`Invalid method parameters.`);
      }
      if (method === "message/stream" || method === "tasks/resubscribe") {
        const params = rpcRequest.params;
        const agentCard = await this.requestHandler.getAgentCard();
        if (!agentCard.capabilities.streaming) {
          throw chunk6NYM5ZKZ_cjs.A2AError.unsupportedOperation(`Method ${method} requires streaming capability.`);
        }
        ...
        switch (method) {
          ...
          case "agent/getAuthenticatedExtendedCard":
            ...
            break;
          default:
            throw chunk6NYM5ZKZ_cjs.A2AError.methodNotFound(method);
        }
        return {
          jsonrpc: "2.0",
          id: requestId,
      ...
      let a2aError;
      if (error instanceof chunk6NYM5ZKZ_cjs.A2AError) {
        a2aError = error;
      } else {
...
// src/core/server/authentication/user.ts
...
exports.UnauthenticatedUser = UnauthenticatedUser;
